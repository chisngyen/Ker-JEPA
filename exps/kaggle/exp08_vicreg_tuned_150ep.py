# =============================================================================
# exp08_vicreg_tuned_150ep.py
# VICReg + Student-t KSD — 150 SSL epochs, TUNED for longer training
#
# DIFFERENCES vs exp07 (same-config scale-up):
#   · batch: 128 → 256   — larger batch improves covariance estimation
#                          (VICReg paper uses 2048; linear LR scaling applies)
#   · LR:   5e-4 → 1e-3  — linear scale with batch (256/128 × 5e-4 = 1e-3)
#   · WD:   0.05  → 0.04  — slight reduction; larger LR already regularizes
#   · warmup: 10ep → 15ep — longer warmup for higher LR stability
#   · proj_dim: 512 (unchanged)
#
# HYPOTHESIS: larger batch → better per-step covariance estimates →
# faster collapse prevention → faster LP accuracy growth.
#
# TARGET: >88% in 150 SSL + 50 LP epochs
# =============================================================================
...
SSL_EPOCHS  = 150
LP_EPOCHS   = 50
BATCH_SIZE  = 256       # ← doubled vs exp07
LR_SSL      = 1e-3      # ← linear-scaled with batch (2× batch → 2× LR)
WD          = 0.04
PROJ_DIM    = 512
W_INV, W_VAR, W_COV = 25.0, 25.0, 1.0
W_KSD       = 0.05
NU          = 3.0
WARMUP_EP   = 15        # longer warmup for higher LR
...
def main():
    print("\n" + "=" * 60)
    print("  EXP-08: VICReg + KSD-ST TUNED  |  150 SSL + 50 LP  |  H100")
    print(f"  batch={BATCH_SIZE}  LR={LR_SSL}  warmup={WARMUP_EP}ep  λ_ksd={W_KSD}")
    print("=" * 60)
    ...
    ckpt = f"{SAVE_DIR}/exp08_vicreg_tuned_150ep.pth"
    torch.save(raw_model.state_dict(), ckpt)
    print(f"\n  Checkpoint: {ckpt}")

    best_acc = run_lp(raw_model.backbone, device)

    report = {"method": "EXP08_VICReg_KSD_ST_150ep_tuned", "ssl_epochs": SSL_EPOCHS,
              "batch_size": BATCH_SIZE, "lr": LR_SSL, "wd": WD,
              "warmup_ep": WARMUP_EP, "w_ksd": W_KSD, "nu": NU,
              "linear_probe_acc": best_acc, "paper_sota": 91.90,
              "delta": round(best_acc - 91.90, 2)}
    with open("/kaggle/working/results_exp08.json", "w") as f:
