...
# === LOSS: FINITE SLICED KSD (Table 1 Variant) ===
class SlicedKSDLoss(nn.Module):
    def __init__(self, n_slices=1024, sigma=1.0):
        super().__init__()
        self.n_slices = n_slices
        self.sigma = sigma
        
        # Quadrature setup (Hermite knots)
        from scipy.special import roots_hermite
        knots, weights = roots_hermite(21) # 21 knots
    ...
    def forward(self, z):
        ...
        theta = F.normalize(torch.randn(self.n_slices, d, device=z.device), dim=1)
        proj = z @ theta.T # [Batch, Slices]
        s = -proj / (self.sigma**2) # Score in projected space
        
        # Stein ECF terms
        args = proj.unsqueeze(-1) * self.knots
        w = self.knots.expand_as(args)
        
        # real = s * cos(args) - w * sin(args)
        # imag = s * sin(args) + w * cos(args)
        real = s.unsqueeze(-1) * torch.cos(args) - w * torch.sin(args)
        imag = s.unsqueeze(-1) * torch.sin(args) + w * torch.cos(args)
        
        disc = torch.mean(real, 0).pow(2) + torch.mean(imag, 0).pow(2)
        sksd = torch.sum(disc * self.weights, dim=-1).mean()
        return sksd
...
# === MAIN LOGIC ===
def main():
    print("\n" + "🚀 KAG_B5: KERJEPA SLICED KSD | 50+50 EPOCHS | H100".center(60, "="))
    ...
    loss_fn = SlicedKSDLoss().to(device)
    ...
    torch.save(model.state_dict(), f"{SAVE_DIR}/kag_kerjepa_sliced.pth")
    ...
    report = {"method": "KerJEPA_Sliced_KSD", "pretraining_epochs": ssl_epochs, "linear_probe_acc": best_acc}
    with open("/kaggle/working/results_kerjepa_sliced.json", "w") as f:
        json.dump(report, f, indent=2)

    print("\n" + "═"*60)
    print(f"🎯 FINAL SLICED REPORT: {best_acc:.2f}%")
    print("═"*60)
